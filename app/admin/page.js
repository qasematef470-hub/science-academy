  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // 🟢 Real-time listener: if the super admin deletes this instructor, kick them out immediately
        unsubscribeSnapshot = onSnapshot(doc(db, 'users', user.uid), async (docSnap) => {
          if (!docSnap.exists() || docSnap.data().role !== 'admin' || docSnap.data().isLocked) {
            await signOut(auth);
            document.cookie = "firebaseToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    ...
    <div className={`min-h-screen font-sans dir-rtl transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-slate-900'}`} dir="rtl">
      ...
      <main className={`flex-1 min-h-screen flex flex-col transition-all duration-300 ${isSidebarOpen ? 'md:mr-64' : 'md:mr-20'}`}>
        ...
        <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-fade-in relative z-10 overflow-x-hidden">
          ...
          {activeTab === 'courses' && <CoursesTab courses={myCourses} onRefresh={() => loadInitialData(auth.currentUser.uid)} isDarkMode={isDarkMode} adminData={adminData} onOpenStructure={() => setShowStructureModal(true)} />}
