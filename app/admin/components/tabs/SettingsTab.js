...
export default function SettingsTab({ myCourses, isDarkMode, adminData }) {
    ...
    const handleAddAdmin = async () => {
        if (!adminEmail || !adminPass || !adminName) return alert('⚠️ الرجاء إكمال بيانات المسؤول الجديد');
        setIsProcessingAdmin(true);
        const res = await addNewAdmin(adminEmail, adminPass, adminName);
        if (res.success) {
            alert(res.message);
    ...
    if (!selectedCourseId) {
        return (
            <div className="space-y-8 animate-fade-in">
                ...
                {/* === منطقة المدير العام (Super Admin) === */}
                <div className="flex items-center gap-2 pt-10 border-t border-gray-200 dark:border-gray-700 mt-10">
                    <h3 className={`text-xl font-black ${theme.textMain}`}>👑 إدارة المسؤولين والأمان (Super Admin)</h3>
                </div>

                {isSuperAdmin ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 animate-scale-in">
                        {/* إضافة مسؤول جديد */}
                        <div className={`p-6 rounded-3xl border shadow-sm ${theme.card}`}>
                            <h4 className={`font-bold mb-4 flex items-center gap-2 ${theme.textMain}`}><span>🧑‍💻</span> إضافة مسؤول جديد</h4>
                            <div className="space-y-3">
                                <input type="text" placeholder="اسم المسؤول" className={`w-full p-3 rounded-xl outline-none border ${theme.input} text-sm font-bold`} value={adminName} onChange={e => setAdminName(e.target.value)} />
                                <input type="email" placeholder="البريد الإلكتروني" className={`w-full p-3 rounded-xl outline-none border ${theme.input} text-sm font-bold`} value={adminEmail} onChange={e => setAdminEmail(e.target.value)} />
                                <input type="text" placeholder="كلمة المرور" className={`w-full p-3 rounded-xl outline-none border ${theme.input} text-sm font-bold`} value={adminPass} onChange={e => setAdminPass(e.target.value)} />
                                <button
                                    onClick={handleAddAdmin}
                                    disabled={isProcessingAdmin}
                                    className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-black transition shadow-[0_4px_20px_rgba(16,185,129,0.3)] disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    {isProcessingAdmin ? 'جاري الإضافة...' : 'إضافة المسؤول ➕'}
                                </button>
                            </div>
                        </div>
                        ...
                        {/* صلاحيات الأمان */}
                        <div className={`p-6 rounded-3xl border shadow-sm ${theme.card}`}>
                            <h4 className={`font-bold mb-4 flex items-center gap-2 ${theme.textMain}`}><span>🔑</span> إجراءات خطيرة (Danger Zone)</h4>
                            <div className="space-y-4">
                                <p className={`text-xs font-bold leading-relaxed ${theme.textSec}`}>
                                    كـ مدير عام (Super Admin)، أنت الوحيد الذي يملك صلاحيات التحكم الكاملة في حسابات المنصة:
                                </p>

                                <div className="p-4 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-600 dark:text-orange-400 text-sm font-bold flex items-start gap-3">
                                    <span>⚠️</span>
                                    <p className="leading-snug">تغيير كلمة المرور لأي حساب (أدمن أو طالب) متاح برمجياً لك فقط، ويمكن تنفيذه من خلال نوافذ التعديل في النظام.</p>
                                </div>

                                <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 text-sm font-bold flex items-start gap-3">
                                    <span>🗑️</span>
                                    <p className="leading-snug">حذف حسابات المسؤولين الآخرين يتم برمجياً عن طريق مطابقة إيميل المدير العام ولا يُسمح به لأي مدير تنفيذي آخر.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ) : (
                    <div className={`mt-6 p-10 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center text-center ${isDarkMode ? 'bg-red-500/5 border-red-500/20 text-red-400' : 'bg-red-50 text-red-500 border-red-200'}`}>
                        <div className="text-6xl mb-6 drop-shadow-lg">🔒</div>
                        <h4 className="text-2xl font-black mb-3">منطقة محظورة</h4>
                        <p className="font-bold opacity-80 max-w-md leading-relaxed">
                            إدارة المسؤولين وإضافة مدراء جدد، وتغيير كلمات السر المتقدمة هي صلاحيات مخصصة حصرياً للمدير العام (Super Admin).
                        </p>
                    </div>
                )}
